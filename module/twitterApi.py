...
listenlist = list(
    data_read_auto(listenlistconfig, default=['1109751762733301760']))
    ...
    for userid in listenlist:
        if i >= page * 5 and i < (page + 1) * 5:
            ...
            if userinfo is not None:
                msg += '\n{0}({1}),{2}'.format(userinfo['screen_name'],
                                               userinfo['name'],
                                               userinfo['id'])
            else:
...
def Run():
    ...
    while True:
        ...
        if run_info['errorCount'] > 0:
            exp_send('推特流异常正在尝试重启，' + '第 ' + str(run_info['errorCount']) +
                     ' 次尝试...')
        try:
            # 创建监听流
        ...
        if run_info['errorCount'] > 0 and int(
                time.time()) - run_info['lastRunTime'] > 300:
...
def runTwitterApiThread():
    # 推特流维护线程
    run_info['ListenThread'] = threading.Thread(group=None,
                                                target=Run,
                                                name='ListenThread',
                                                daemon=True)
