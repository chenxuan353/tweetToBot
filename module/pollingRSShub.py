...
def Priority_canRun(path: str, defaultpriority: int = defaultpriority):
    ...
    if path not in nowpriority:
        nowpriority[path] = prioritylist[
            path] if path in prioritylist else random.randint(
                1, defaultpriority)
    ...
    if nowpriority[path] == 0:
        nowpriority[path] = prioritylist[
            path] if path in prioritylist else defaultpriority
        return True
    return False
    ...
    try:
        while True:
            if run_info['keepRun']:
                ...
                if count % 10 == 0:
                    nowtime = time.time()
                    logger.info("RSShub 完整检测，{0}s".format(
...
def runPollingRSShubThread():
    run_info['ListenThread'] = threading.Thread(
        group=None, target=Run, name='PollingRSShubThread_thread', daemon=True)
