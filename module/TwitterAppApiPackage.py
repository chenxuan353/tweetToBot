...
class TwitterAppApiPackage:
    ...
    def users_timeline(self,
        ...
        try:
            if autoid:
                tweets = self.api.user_timeline(id=autoid,
                                                since_id=since_id,
                                                tweet_mode='extended')
            elif user_id:
...
class PollingTwitterApps:
    ...
    def __init__(self, consumers: list):
        ...
        for consumer in self.consumers:
            self.apps.append(TwitterAppApiPackage(consumer[0], consumer[1]))
        appl = len(self.apps)
        for key in self.waitTime:
            self.waitTime[key] = int(
                (1 / (self.allowFunname[key] * appl)) * 100) / 100 + 0.5
