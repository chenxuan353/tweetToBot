...
legalPermissionList = {'perms': {}, 'roles': {}, 'default': {}}
...
def permRegisterGroup(modulename: str,
                      groupname: str,
                      groupnick: str,
                      groupdes: str,
                      permlist: dict = {}):
...
def permRoleMerge(rolename: str, permgroup: str, permlist: set):
    ...
    if type(permlist) != set:
        raise Exception("角色注册异常,{0} · {1}，权限列表类型不为集合(set)".format(
            rolename, permgroup))
    if permgroup in legalPermissionList['roles'][rolename]:
        permlist = permlist.union(
            legalPermissionList['roles'][rolename][permgroup])
...
def authRegisterGroup(modulename: str, groupname: str, groupnick: str,
                      groupdes: str):
    """
        注册插件权限组
    """
...
def authRegisterDefaultPerm(groupname: str, perm: str,
                            msgtype: PlugMsgTypeEnum):
    """
        注册插件默认权限
        msgtype:默认授予对象
    """
...
#核心函数
def perm_add(bottype: str, botuuid: str, msgtype: str, uuid: str,
             groupname: str, perm: str, authunit: dict) -> tuple:
    """
        设定权限
            authunit = {
                'bottype':bottype,
    ...
    if not legalPermHas(groupname, perm):
        return (False, "{groupname}->{perm},此权限不存在".format(groupname=groupname,
                                                           perm=perm))
    dictSet(permissionList,
            bottype,
            botuuid,
            msgtype,
            uuid,
...
def perm_del(bottype: str, botuuid: str, msgtype: str, uuid: str,
    ...
    if not dictHas(permissionList, bottype, botuuid, msgtype, uuid, groupname,
                   perm):
        return (False, "{groupname}->{perm},权限未曾授权".format(groupname=groupname,
                                                           perm=perm))
...
def perm_get(bottype: str, botuuid: str, msgtype: str, uuid: str,
             groupname: str, perm: str) -> tuple:
    """
    ...
    if not dictHas(permissionList, bottype, botuuid, msgtype, uuid, groupname,
        ...
        return (False, "{groupname}->{perm},权限未曾授权".format(groupname=groupname,
                                                           perm=perm))
    return (True,
            permissionList[bottype][botuuid][msgtype][uuid][groupname][perm])


def perm_has(bottype: str, botuuid: str, msgtype: str, uuid: str,
             groupname: str, perm: str) -> bool:
    """
        检查授权单元是否存在
    """
    global permissionList
...
def perm_uuidGet(bottype: str, botuuid: str, msgtype: str, uuid: str) -> dict:
    ...
    default: dict = dictGet(legalPermissionList['default'],
                            msgtype,
                            default=dict()).copy()
...
def perm_uuidAuthGet(bottype: str, botuuid: str, msgtype: str,
                     uuid: str) -> dict:
    return dictGet(permissionList,
                   bottype,
                   botuuid,
                   msgtype,
                   uuid,
...
#扩展函数
def perm_checkdeny(bottype: str, botuuid: str, msgtype: str, uuid: str,
                   groupname: str, perm: str) -> bool:
    """
        检查权限是否被显式禁用
    """
    ...
    def authcheck(authunit: dict):
        ...
        if 'expireTimestamp' in authunit:
            if authunit['expireTimestamp'] <= 0 or nowtime > authunit[
                    'expireTimestamp']:
                return True
        else:
            return True
    ...
    if authcheck(
            dictGet(permissionList, bottype, botuuid, msgtype, uuid, groupname,
                    '-*')):
        return True
    if authcheck(
            dictGet(permissionList, bottype, botuuid, msgtype, uuid, groupname,
...
def perm_check(bottype: str, botuuid: str, msgtype: str, uuid: str,
               groupname: str, perm: str) -> bool:
    global legalPermissionList, permissionList
    if perm.startswith('-'):
...
def perm_deny(bottype: str, botuuid: str, msgtype: str, uuid: str,
              groupname: str, perm: str, authunit: dict) -> tuple:
...
def perm_allow(bottype: str, botuuid: str, msgtype: str, uuid: str,
               groupname: str, perm: str, authunit: dict) -> tuple:
    if perm.startswith('-'):
        perm = perm[1:]
    if perm_checkdeny(bottype, botuuid, msgtype, uuid, groupname, perm):
        return (False,
                "{groupname}->{perm},授权被禁用请检查是否存在“-*”或“-{perm}”授权".format(
                    groupname=groupname, perm=perm))
...
def authObjGetList(bottype: str,
                   botuuid: str,
                   msgtype: str,
                   uuid: str,
                   page: int = 1) -> str:
...
def authGet(bottype: str, botuuid: str, msgtype: str, uuid: str,
            groupname: str, perm: str) -> dict:
    """
        获取授权单元，不存在时返回空字典
            authunit = {
                'bottype':bottype,
...
def authBaleObjectRecognition(bottype: str, botuuid: str, msgtype: str,
                              uuid: str, standmsg: str) -> dict:
    """
...
def authAllow(bottype: str,
              botuuid: str,
              msgtype: str,
              uuid: str,
              groupname: str,
              perm: str,
              objectRecognition: dict,
              overlapping=True,
              **kw) -> tuple:
    """
        授权
    ...
    if overlapping and authcheck(
            dictGet(permissionList, bottype, botuuid, msgtype, uuid, groupname,
                    perm)):
        return (False, '授权重复！')
    createTimestamp = (kw['createTimestamp']
                       if 'createTimestamp' in kw else time.time())
    ...
    return perm_allow(bottype, botuuid, msgtype, uuid, groupname, perm,
                      authunit)


def authRemoval(bottype: str, botuuid: str, msgtype: str, uuid: str,
                groupname: str, perm: str) -> tuple:
    """
        授权移除，仅对正常授权有效
    """
...
def authDeny(bottype: str,
    ...
    if overlapping and dictHas(permissionList, bottype, botuuid, msgtype, uuid,
                               groupname, perm):
        return (False, '授权重复！')
    createTimestamp = (kw['createTimestamp']
                       if 'createTimestamp' in kw else time.time())
    ...
    return perm_deny(bottype, botuuid, msgtype, uuid, groupname, perm,
                     authunit)


def authDenyRemoval(bottype: str, botuuid: str, msgtype: str, uuid: str,
                    groupname: str, perm: str) -> tuple:
    if not legalPermHas(groupname, '-' + perm):
        return (False, "{groupname}->{perm},此权限不存在".format(groupname=groupname,
                                                           perm='-' + perm))
...
def authCheck(bottype: str, botuuid: str, msgtype: str, uuid: str,
              groupname: str, perm: str) -> bool:
    """
        权限检查，检查是否拥有指定权限
    """
...
def authCheckDeny(bottype: str, botuuid: str, msgtype: str, uuid: str,
                  groupname: str, perm: str) -> bool:
    """
        权限检查，检查是否拥有指定权限
    """
