    ...
    def decorate(func):
        ...
        async def wrapper(session: Session) -> PlugMsgReturn:
            ...
            if not session.authCheck(
                    ...
                    PluginsManageList[func.__module__].groupname, 'plugopen'):
                return PlugMsgReturn.Ignore
            ...
            if not res[0]:
                ...
                errdes = res[2]
                argdes = res[3]
                session.send("{nick}，{errdes}，参数说明 {des}".format(nick=nick,
                                                                 errdes=errdes,
                                                                 des=argdes))
                return PlugMsgReturn.Intercept
        ...
        if func.__module__ in PluginsManageList:
            if DEBUG:
                logger.info('函数注册{module}.{name}·{des}'.format(
                    module=wrapper.__module__, name=wrapper.__name__, des=des))
            #权限注册(此处不需要)
            #if bindperm:
            ...
            PluginsManageList[func.__module__].addFunc(
                wrapper,
                PluginsManage.baleFuncInfo(wrapper.__module__,
                                           wrapper.__name__, msgfilter,
                                           argfilter, des, limitMsgType,
                                           allow_anonymous, at_to_me),
                checkmodule=False)
        else:
...
def __eventArrives(even: StandEven) -> PlugMsgReturn:
    ...
    session: Session = sessionManagement.getSession(even.bottype, even.botuuid,
                                                    even.botgroup, even.uuid,
                                                    even)
            ...
            if res == PlugMsgReturn.Intercept:
                if DEBUG:
                    logger.info('插件{modulename}·{name}拦截了这条消息'.format(
                        modulename=plug.modulename, name=plug.name))
                return PlugMsgReturn.Intercept
    if DEBUG:
