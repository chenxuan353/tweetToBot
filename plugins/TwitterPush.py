...
def _(plug: PluginsManage):
    if plug:
        ...
        plug.registerPerm('manageself',
                          des='管理自己的权限',
                          defaultperm=PlugMsgTypeEnum.allowall)
        plug.registerPerm('cacheinfo',
                          des='获取缓存信息的权限',
                          defaultperm=PlugMsgTypeEnum.allowall)
...
@on_message(msgfilter='取消转推授权',
            bindsendperm='manageuse',
            des='取消转推授权 - 取消转推授权')
async def _(session: Session):
...
@on_message(msgfilter='定向清空转推列表',
            argfilter=argfilter,
            bindsendperm='manage',
            des='定向清空转推列表 消息来源标识 消息来源ID - 定向清空转推列表')
async def _(session: Session):
    msgtype = session.filterargs['msgtype']
    uuid = session.filterargs['uuid']
    res = pushlist.delPushunitFromPushTo(session.bottype, session.botuuid,
                                         msgtype, uuid)
    session.send(res[1])
...
@on_message(msgfilter='定向清空转推对象',
            argfilter=argfilter,
            bindsendperm='manage',
            des='定向清空转推对象 用户名 - 定向清空转推对象')
async def _(session: Session):
...
@on_message(msgfilter='添加辅助转推',
            argfilter=argfilter,
            bindsendperm='manage',
            des='添加辅助转推 用户名 - 添加辅助转推')
async def _(session: Session):
    userinfo = session.filterargs['userinfo']
...
@on_message(msgfilter='删除辅助转推',
            argfilter=argfilter,
            bindsendperm='manage',
            des='删除辅助转推 用户名 - 删除辅助转推')
async def _(session: Session):
    userinfo = session.filterargs['userinfo']
...
@on_message(msgfilter='辅助转推列表',
            argfilter=argfilter,
            bindsendperm='manage',
            des='辅助转推列表 页码 - 辅助转推列表')
async def _(session: Session):
    page = session.filterargs['page']
...
@on_message(msgfilter='启动辅助监听',
            bindsendperm='manage',
            des='启动辅助监听 - 启动推送的辅助监听')
async def _(session: Session):
    apisetStreamOpen(True)
    session.send('已响应')


@on_message(msgfilter='关闭辅助监听',
            bindsendperm='manage',
            des='关闭辅助监听 - 关闭推送的辅助监听')
async def _(session: Session):
...
@on_message(msgfilter='(获取推文)|(看推)',
            argfilter=argfilter,
            bindperm='use',
            bindsendperm='cacheinfo',
            des='获取推文 推文ID或推文链接 - 获取指定推文',
            at_to_me=False)
async def _(session: Session):
...
@on_message(msgfilter='推送优先级设置列表',
            argfilter=argfilter,
            bindsendperm='manage',
            des='推送优先级设置列表 - 轮询优先级列表')
async def _(session: Session):
    page = session.filterargs['page']
...
argfilter.addArg('userinfo',
                 ...
                 prefunc=functools.partial(getRealTweetUserID, useapi=False),
                 verif='other')
argfilter.addArg('value',
                 '优先级',
                 '优先级设置，范围1-15',
                 prefunc=(lambda arg: int(arg)
                          if arg.isdigit() and 0 < int(arg) <= 15 else None),
                 verif='intnozero',
                 vlimit={'': 1})


@on_message(msgfilter='设置推送优先级',
            argfilter=argfilter,
            bindsendperm='manage',
            des='设置推送优先级 用户ID 优先级 - 设置轮询优先级')
async def _(session: Session):
...
@on_message(msgfilter='查询推特用户',
            argfilter=argfilter,
            bindperm='use',
            bindsendperm='cacheinfo',
            des='查询推特用户 用户ID或用户名 - 查询指定推特用户的信息',
            at_to_me=False)
async def _(session: Session):
    userinfo = session.filterargs['userinfo']
    msg = "UID：{0}\n用户名：{1}\n用户昵称：{2}\n头像：{3}\n描述：{4}\n".format(
        ...
        userinfo['name'],
        SendMessage().append(
            SendMessage.baleImgObj(
                userinfo['profile_image_url'])).toStandStr(),
        userinfo['description'],
    )
    try:
        msg += "粉丝数：{0}\n".format(userinfo['followers_count'])
        msg += "关注数：{0}\n".format(userinfo['friends_count'])
        msg += "发推数(包括转发)：{0}\n".format(userinfo['statuses_count'])
        msg += "账户创建时间：{0}".format(
            time.strftime("%Y{0}%m{1}%d{2} %H:%M:%S",
                          time.localtime(int(userinfo['created_at']))).format(
                              '年', '月', '日'))
    except:
        pass
    session.send(msg)
...
def getTweetsList(tweets, page: int = 1):
    ...
    for i in range(lll, -1, -1):
        ...
        if j >= page * 5 and j < (page + 1) * 5:
            msg += '\n' + tweetevendeal.tweetToMsg(tweets[i],
                                                   simple=True).toStandStr()
    msg += '\n当前页{0}/{1} (共{2}个推文)'.format(page + 1, int(lll / 5) + 1, lll)
    return msg
...
@on_message(msgfilter='推文列表',
            ...
            bindperm='use',
            bindsendperm='cacheinfo',
            des='推文列表 用户ID或用户名 页码 - 查询指定推特用户的推文列表',
            at_to_me=False)
async def _(session: Session):
    userinfo = session.filterargs['userinfo']
    page = session.filterargs['page']
    if userinfo is None:
        res = pushlist.getLitsFromPushTo(session.bottype, session.botuuid,
                                         session.botgroup, session.uuid)
        if not res:
    ...
    session.send("{0} 的推文列表：{1}".format(
        (userinfo['name'] if userinfo is not None else userid), msg))
...
argfilter.addArg('userinfo',
                 ...
                 prefunc=getRealTweetUserID,
                 verif='other')
argfilter.addArg('nick', '推送昵称', '推送昵称', verif='str', vlimit={'': ''})
argfilter.addArg('des', '推送描述', '推送描述', verif='str', vlimit={'': ''})


@on_message(msgfilter='(加推)|(addone)|(D一个)',
            argfilter=argfilter,
            des='加推 用户名 昵称(可选) 描述(可选) - 增加一个推送订阅，别名addone、D一个',
            sourceAdmin=True,
            bindperm='use',
...
async def _(session: Session):
    ...
    pushconfig = pushlist.baleForConfig(nick=nick, des=des)
    unit = pushlist.baleToPushUnit(session.bottype,
                                   session.botuuid,
                                   session.botgroup,
                                   session.uuid,
                                   userinfo['id_str'],
...
@on_message(msgfilter='(减推)|(delone)|(少D一个)',
            argfilter=argfilter,
            des='减推 用户名 - 减少一个推送订阅，别名delone、少D一个',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself')
async def _(session: Session):
    userinfo = session.filterargs['userinfo']
    unit = pushlist.findPushunit(session.bottype, session.botuuid,
                                 session.botgroup, session.uuid,
                                 userinfo['id_str'])
    if unit is None:
...
def getSpyList(spylist, page: int = 1):
    ...
    for spyunit in spylist:
        ...
        des = spyunit['pushconfig']['unit']['des'] if spyunit['pushconfig'][
            'unit']['des'] else "未设置"
        if i >= page * 5 and i < (page + 1) * 5:
            if userinfo is None:
                msg += "\n{0},{1},{2}".format((nick if nick else "未设置"),
                                              spyunit['spyuuid'], des)
            else:
                msg += "\n{0},{1},{2}".format(
                    (nick if nick else userinfo['name']),
                    userinfo['screen_name'], des)
        i += 1
...
async def _(session: Session):
    page = session.filterargs['page']
    l = pushlist.getLitsFromPushTo(session.bottype, session.botuuid,
                                   session.botgroup, session.uuid)
    msg = getSpyList(l, page)
    session.send(msg)


@on_message(msgfilter='(清空推送)|(delallpush)',
            des='清空推送 - 清空推送列表，别名delallpush',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself')
async def _(session: Session):
...
def getPushToSetting(config: dict, kind: str = 'basic') -> str:
    ...
    if kind == 'basic':
        value = config['upimg']
        res += '\n图片:{0}'.format((value if value not in (0, 1, '') else {
            0: '关闭',
            1: '开启',
            '': '未定义'
        }[value]))
    ...
    for key, value in config['push'].items():
        if key in attrlist[kind]:
            res += '\n{0}:{1}'.format(attrlist[kind][key],
                                      (value if value not in (0, 1, '') else {
                                          0: '关闭',
                                          1: '开启',
                                          '': '未定义'
                                      }[value]))
...
@on_message(msgfilter='转推设置列表',
            argfilter=argfilter,
            des='转推设置列表 配置界面 - 界面支持四个选项',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself')
async def _(session: Session):
    pagename = session.filterargs['pagename']
    pushto = pushlist.getPushTo(session.bottype, session.botuuid,
                                session.botgroup, session.uuid)
    if pushto is None:
...
argfilter.addArg(
    ...
    vlimit={
        ...
        'ai_passive_reply_to_user': 'ai_passive_reply_to_user',
        ...
        '粉丝增加': 'change_followers',
        '粉丝数': 'change_followers'
    })
argfilter.addArg('value',
                 '要设置的值',
                 '需要设置的值',
...
@on_message(msgfilter='转推设置',
            argfilter=argfilter,
            des='推送设置 属性 属性值- 设置全局推送设置',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself')
async def _(session: Session):
    ...
    value = session.filterargs['value']
    res = pushlist.PushTo_setAttr(session.bottype, session.botuuid,
                                  session.botgroup, session.uuid, key, value)
    if not res[0]:
        session.send(res[1])
        return
    session.send(res[1])
...
@on_message(msgfilter='转推单元设置',
            argfilter=argfilter,
            des='推送单元设置 对象ID 属性 属性值- 设置单元推送设置',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself')
async def _(session: Session):
    ...
    key = session.filterargs['key']
    value = session.filterargs['value']
    res = pushlist.setPushunitAttr(session.bottype, session.botuuid,
                                   session.botgroup, session.uuid,
                                   userinfo['id_str'], key, value)
    if not res[0]:
        session.send(res[1])
        return
    session.send(res[1])
...
@on_message(msgfilter='转推单元设置列表',
            argfilter=argfilter,
            des='转推单元设置列表 用户名 配置界面 - 界面支持四个选项',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself')
async def _(session: Session):
    ...
    if pushto is None:
        session.send('设置不存在，请添加推送后再试。')
        return
    pushunit = pushlist.findPushunit(session.bottype, session.botuuid,
                                     session.botgroup, session.uuid,
                                     userinfo['id_str'])
    if pushunit is None:
...
argfilter = PlugArgFilter()
argfilter.addArg('tweetid', '正整数', '推文ID或任意正整数', verif='uint')
