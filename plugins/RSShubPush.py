...
@on_message(msgfilter='启动RSS监听',
            bindsendperm='manage',
            des='启动RSS监听 - 启动RSS推送的主要监听')
async def _(session: Session):
    setStreamOpen(True)
    session.send('已响应')


@on_message(msgfilter='关闭RSS监听',
            bindsendperm='manage',
            des='关闭RSS监听 - 启动RSS推送的主要监听')
async def _(session: Session):
    setStreamOpen(False)
    session.send('已响应')


@on_message(msgfilter='RSS订阅授权',
...
@on_message(msgfilter='取消RSS订阅授权',
            bindsendperm='manageuse',
            des='取消RSS订阅授权 - 取消RSS订阅授权')
async def _(session: Session):
...
@on_message(msgfilter='RSS优先级设置列表',
            argfilter=argfilter,
            bindsendperm='manage',
            des='RSS优先级设置列表 - RSS优先级设置列表')
async def _(session: Session):
...
@on_message(msgfilter='设置RSS优先级',
            argfilter=argfilter,
            bindsendperm='manage',
            des='设置RSS优先级 路径 优先级 - 设置轮询优先级')
async def _(session: Session):
...
argfilter.addArg('unitdes', '项目描述', '推送的项目描述', verif='str', vlimit={'': ''})


@on_message(
    msgfilter='订阅',
    argfilter=argfilter,
    des='订阅 路径 昵称(可选) 推送描述(可选) 选项(可选)- 增加一个RSS订阅,路径为根域名后的相对路径,无法输入的路径用·替代/',
    sourceAdmin=True,
    bindperm='use',
...
async def _(session: Session):
    ...
    if not res[0]:
        ...
        return
    options = res[1]
    pushconfig = pushlist.baleForConfig(nick=nick,
                                        unitdes=unitdes,
                                        options=options)
...
@on_message(msgfilter='取消订阅',
            argfilter=argfilter,
            des='取消订阅 路径 - 减少一个推送订阅',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself')
async def _(session: Session):
...
@on_message(msgfilter='订阅源解码',
            argfilter=argfilter,
            des='订阅源解码 路径 - 获取解码后的路径',
            sourceAdmin=True,
            bindperm='use',
            bindsendperm='manageself',
            at_to_me=False)
...
def getSpyList(spylist, page: int = 1):
    ...
    for spyunit in spylist:
        nick = spyunit['pushconfig']['nick'] if spyunit['pushconfig'][
            'nick'] else "未设置"
        des = spyunit['pushconfig']['unitdes'] if spyunit['pushconfig'][
            'unitdes'] else "未设置"
        if des == "未设置":
