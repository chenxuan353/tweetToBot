...
async def _(session:Session):
    ...
    dictSet(streamlist,session.bottype,session.botgroup,session.botuuid,session.uuid,transtarget,obj=streamobj)
...
def getstreamlist(streamlist,page:int = 1):
    ...
    for streamunit in streamlist.values():
        if i >= page*5 and i < (page+1)*5:
            msg += '\n{0},{1},{2}->{3}'.format(streamunit['transtarget'],streamunit['engine'],streamunit['source'],streamunit['target'])
        i += 1
...
@on_message(msgfilter='([！!]清空流式翻译列表)',des='清空流式翻译列表 - 清空流式翻译列表',bindsendperm='streamtrans',sourceAdmin=True,at_to_me=False)
async def _(session:Session):
    res = dictGet(streamlist,session.bottype,session.botgroup,session.botuuid,session.uuid)
    if res is None:
        session.send("流式翻译列表为空！")
