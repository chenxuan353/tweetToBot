...
@on_message(msgfilter='(反馈)|(feedback)',bindperm='feedback',bindsendperm='feedback',des='反馈 反馈内容 - 反馈信息,别名feedback')
async def _(session:Session):
    ...
    if feedbackunit is None:
        ...
        feedbackunit['simpletext'] = feedbackmsg[:15].replace('\r',' ').replace('\n',' ')
    ...
    msg = "反馈ID:{0}\n消息来源：{1}-{2}-{3}({4})\n内容：\n{5}\n反馈时间：{6}".format(
            ...
            nick,
            feedbackmsg,
            time.strftime("%Y{0}%m{1}%d{2} %H:%M:%S",time.localtime(feedbackunit['timestamp'])).format('年','月','日')
            )
...
argfilter.addArg(
    ...
    '反馈ID',
    '需要输入有效反馈ID',
    prefunc=(lambda arg:feedbacktmemory.find((lambda item,val: str(item['id'])==val),arg.strip())),
    verif='other',
    canSkip=False
    )
@on_message(msgfilter='(处理反馈)|(dealfeedback)',argfilter=argfilter,bindsendperm='manage',des='处理反馈 反馈ID 处理信息 - 处理反馈信息,别名dealfeedback')
async def _(session:Session):
    ...
    msg = "来自 ID:{0} 的反馈回复\n上一条消息：\n{1}\n回复内容：\n{2}".format(
            feedbackunit['id'],
            feedbackunit['replay'][-1]['text'][:30].strip(),
            replaymsg
            )
    send_msg(
            feedbackunit['bottype'],
            feedbackunit['botuuid'],
            feedbackunit['botgroup'],
            feedbackunit['uuid'],
            feedbackunit['sourceObj'],
            SendMessage(msg)
        )
    session.send('回复已发送')

@on_message(msgfilter='(反馈完成)|(反馈处理完成)',argfilter=argfilter,bindsendperm='manage',des='反馈完成 反馈ID - 处理反馈信息,别名反馈处理完成')
async def _(session:Session):
    feedbackunit = session.filterargs['unit']
    ...
    for i in range(length - 1,-1,-1):
        if cout >= (page-1)*5 and cout < (page)*5:
            ...
            if feedbackunit['dealcomplate']:
                dealmsg = "已完成"
            s = s + "{0},{1},{2}".format(feedbackunit['id'],dealmsg,feedbackunit['simpletext'])
        cout = cout + 1
