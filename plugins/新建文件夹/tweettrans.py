# -*- coding: UTF-8 -*-
from nonebot import on_command as nonebot_on_command, CommandSession,NoticeSession,on_notice,permission as perm
...
import traceback
import re
import module.permissiongroup as permgroup
import config
...
try:
    ...
    logger.warning('推特推送组件未找到，部分功能无法使用。')
    pass
#线程池
...
#权限注册
permgroup.perm_initModulePerm(__name__,'烤推模块','用于烤制推特并即时出图的插件',{
        'switch':'操作授权',
        'trans':'烤推权限',
        'template':'模版修改'
    })
perm_check = functools.partial(permgroup.perm_help_check,permgroupname = __name__)
...
@functools.wraps(nonebot_on_command)
def on_command(*args1, **kw1):
    def decorator(func):
        bindperm = kw1['bindperm'] #权限限制表
        @nonebot_on_command(*args1, **kw1)
        @functools.wraps(func)
        async def wrapper(*args, **kw):
            session = args[0]
            #消息过滤
            if session.event['message_type'] not in ('group','private') or \
            session.event['message_type'] == "group" and session.event.sub_type != 'normal':
                return
